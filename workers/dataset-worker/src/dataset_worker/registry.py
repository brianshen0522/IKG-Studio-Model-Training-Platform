...
class WorkerRegistry:
    ...
    def register(self):
        py, torch_v, ultra_v, cuda_v = self._capabilities()
        with psycopg.connect(self.conninfo) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO workers (worker_key, worker_type, hostname, status, python_version, "
                    "torch_version, ultralytics_version, cuda_version, last_heartbeat_at, registered_at, updated_at) "
                    "VALUES (%s,%s,%s,'ONLINE',%s,%s,%s,%s,now(),now(),now()) "
        ...
        while not self._stop.wait(self.interval_s):
            try:
                with psycopg.connect(self.conninfo) as conn:
                    with conn.cursor() as cur:
                        cur.execute(
                            "UPDATE workers SET last_heartbeat_at=now(), status='ONLINE', updated_at=now() "
